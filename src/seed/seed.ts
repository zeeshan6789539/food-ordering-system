 ...
 * Ensure DB is up and .env (or env vars) are set.
 */
...
import { generateOrderId } from '../modules/order/order-id.util';
...
async function run() {
  const ds = new DataSource({
    ...
    entities: [User, Product, ProductVariant, Cart, CartItem, Order, OrderItem],
    synchronize: false,
  });
  ...
  const productsData: Array<{ name: string; description: string | null; basePrice: number; imageUrl: string | null; variants: Array<{ name: string; price: number }> }> = [
